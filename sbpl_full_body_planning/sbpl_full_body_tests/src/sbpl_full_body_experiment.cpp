...
SBPLFullBodyExperiment::SBPLFullBodyExperiment() : ph_("~")
{
  ...
  demonstrated_paths_folder_ = "/tmp";
  ...
  demonstrated_path_pub_ = nh_.advertise<full_body_demonstration::FullBodyTrajectory>("demonstrated_path", 5);
...
bool SBPLFullBodyExperiment::getDemonstratedPaths()
{
  if(!ph_.hasParam("demonstrated_paths_folder"))
  {
    ROS_WARN("[exp] FYI - No demonstrated paths to learn from.");
    return true;
  } 
  ph_.getParam("demonstrated_paths_folder", demonstrated_paths_folder_);
 
  struct stat st;
  if(stat(demonstrated_paths_folder_.c_str(),&st) == 0)
    ROS_INFO("[exp] Demonstrated paths folder was located. (%s)\n",demonstrated_paths_folder_.c_str());
  else
    ROS_ERROR("[exp] Demonstrated paths folder doesn't exist. Not using them. (%s)\n", demonstrated_paths_folder_.c_str());

  DIR *dir;
  struct dirent *ent;
  std::vector<std::string> dem_path_files;
  dir = opendir (demonstrated_paths_folder_.c_str());
  if(dir != NULL)
  {
    while ((ent = readdir(dir)) != NULL)
    {
      printf ("%s\n", ent->d_name);
      dem_path_files.push_back(ent->d_name);
    }
    closedir(dir);
  } 
  else 
  {
    ROS_ERROR("[exp] The demonstrated paths folder is empty.");
    return false;
  }
  
  trajectory_msgs::JointTrajectory traj, btraj;
  full_body_demonstration::FullBodyTrajectory path;
  for(size_t i = 0; i < dem_path_files.size(); ++i)
  {
    // make sure it's a csv file...let's allow for other files in the folder
    if(dem_path_files[i].find_last_of(".") != std::string::npos)
    {
      if(dem_path_files[i].substr(dem_path_files[i].find_last_of(".")+1).compare("csv") != 0)
      {
        ROS_DEBUG("[exp] Skipping '%s' because it's not a csv file.", dem_path_files[i].c_str());
        continue;
      }
    }
    std::string filename = demonstrated_paths_folder_ + "/" + dem_path_files[i];
    if(!parseTrajectoryFile(filename,traj,btraj))
    {
      ROS_ERROR("[exp] Failed to parse trajectory from %s. Skipping it.",filename.c_str());
      continue;
    }
    
    path.trajectory = traj;
    path.body_trajectory = btraj;
    ROS_INFO("[exp] demonstrated path: %s.", dem_path_files[i].c_str());
    demonstrated_path_pub_.publish(path); 
  }
  return true;
}
...
bool SBPLFullBodyExperiment::requestPlan(RobotPose &start_state, std::string name, std::vector<std::vector<double> > &traj)
{
  ...
  req.goal.pose.position.x = loc_map_[exp_map_[name].goal].at(0);
  req.goal.pose.position.y = loc_map_[exp_map_[name].goal].at(1);
  req.goal.pose.position.z = loc_map_[exp_map_[name].goal].at(2);

  btQuaternion btgoal;
  btgoal.setRPY(0.0,0.0,loc_map_[exp_map_[name].goal].at(3));
...
bool SBPLFullBodyExperiment::performAllExperiments()
{
  if(use_current_state_as_start_)
  ...
  for(std::map<std::string,Experiment>::iterator iter = exp_map_.begin(); iter != exp_map_.end(); ++iter)
  {
    ...
    if(!requestPlan(start, iter->second.name, traj))
    {
...
int main (int argc, char **argv)
{
  ...
  if(!exp.getDemonstratedPaths())
    ROS_ERROR("Getting demonstrated paths failed.");
