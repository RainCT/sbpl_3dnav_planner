...
int main(int argc, char** argv){
  ...
  FILE* fLoc = fopen("warehouse_demo_loc.yaml","w");
  FILE* fExp = fopen("warehouse_demo_exp.yaml","w");
  ...
  for(int i=0; i<12; i++){
    if(i%2 == 0){
      double x = 1.5 + palette_across*(1.9-1.5);
      double y = 2.15;
      double z = 0.35 + palette_height*(1.15-0.35)/2.0;
      double theta = 1.57;
      ...
      double x = 0.4;
      double y = 0.4 + shelf_across*(0.8-0.4);
      double z;
      if(shelf_height==0)
        z = 0.4;
